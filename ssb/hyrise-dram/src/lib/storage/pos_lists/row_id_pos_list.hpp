...
class RowIDPosList final : public AbstractPosList, private pmr_vector<RowID> {
 public:
  ...
  /* (1 ) */ RowIDPosList() noexcept(noexcept(allocator_type(&KindMemoryManager::get().get_resource(AllocationPurpose::PosList)))) {}
  /* (1 ) */ explicit RowIDPosList(const allocator_type& allocator) noexcept : Vector(allocator) {}
  /* (2 ) */ RowIDPosList(size_type count, const RowID& value, const allocator_type& alloc = allocator_type(&KindMemoryManager::get().get_resource(AllocationPurpose::PosList)))
      : Vector(count, value, alloc) {}
  /* (3 ) */ explicit RowIDPosList(size_type count, const allocator_type& alloc = allocator_type(&KindMemoryManager::get().get_resource(AllocationPurpose::PosList)))
      : Vector(count, alloc) {}
  /* (4 ) */ template <class InputIt>
  RowIDPosList(InputIt first, InputIt last, const allocator_type& alloc = allocator_type(&KindMemoryManager::get().get_resource(AllocationPurpose::PosList)))
  ...
  /* (8 ) */ RowIDPosList(std::initializer_list<RowID> init, const allocator_type& alloc = allocator_type(&KindMemoryManager::get().get_resource(AllocationPurpose::PosList)))
