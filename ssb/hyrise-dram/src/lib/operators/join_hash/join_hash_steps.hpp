...
template <typename T>
struct Partition {
  pmr_vector<PartitionedElement<T>> elements{PolymorphicAllocator<void>{&KindMemoryManager::get().get_resource(AllocationPurpose::JoinHashMaterialization)}};
  // TODO propagate the allocator into Partition using allocator_traits
  ...
  // due to padding.
  pmr_vector<bool> null_values{PolymorphicAllocator<void>{&KindMemoryManager::get().get_resource(AllocationPurpose::JoinHashMaterialization)}};
};
...
class PosHashTable {
  ...
  using HashTable = ska::bytell_hash_map<HashedType, Offset, std::hash<HashedType>, std::equal_to<HashedType>, PolymorphicAllocator<std::pair<HashedType, Offset>>>;
 ...
 public:
  explicit PosHashTable(const JoinHashBuildMode mode, const size_t max_size)
      : _hash_table(&KindMemoryManager::get().get_resource(AllocationPurpose::JoinHashHashTable)), _pos_lists(max_size + 1, &KindMemoryManager::get().get_resource(AllocationPurpose::JoinHashHashTable)), _mode(mode) {
...
RadixContainer<T> materialize_input(const std::shared_ptr<const Table>& in_table, const ColumnID column_id,
  ...
  auto allocator = PolymorphicAllocator<void>{&KindMemoryManager::get().get_resource(AllocationPurpose::JoinHashMaterialization)};
  // auto allocator = PolymorphicAllocator<void>{};
  auto radix_container = RadixContainer<T>{allocator};
