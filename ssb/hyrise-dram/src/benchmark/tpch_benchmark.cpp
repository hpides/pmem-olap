...
int main(int argc, char* argv[]) {
  ...
  cli_options.add_options()
    ...
    ("tenants", "Number of tenants (increases size of database but not of queries))", cxxopts::value<int>()->default_value("1"))
  ...
  auto scale_factor = cli_parse_result["scale"].as<float>();
  auto tenants = cli_parse_result["tenants"].as<int>();
  ...
  if (jcch) {
    ...
    Assert(tenants == 1, "Multi-Tenant mode not implemented for JCC-H");
    ...
    table_generator = std::make_unique<TPCHTableGenerator>(scale_factor, config, tenants);
    item_runner = std::make_unique<TPCHBenchmarkItemRunner>(config, use_prepared_statements, scale_factor, item_ids, tenants);
  }
  ...
  for (auto tenant = 0; tenant < tenants; ++tenant) {
    for (const auto table_name :
         std::vector{"customer", "lineitem", "nation", "orders", "part", "partsupp", "region", "supplier"}) {
      SQLPipelineBuilder{std::string{"CREATE VIEW "} + table_name + std::to_string(tenant) + " AS SELECT * FROM " +
                         table_name + " WHERE tenant = " + std::to_string(tenant)}
          .create_pipeline()
          .get_result_table();
    }
  }
