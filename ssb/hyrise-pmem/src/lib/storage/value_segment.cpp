...
template <typename T>
ValueSegment<T>::ValueSegment(bool nullable, ChunkOffset capacity) : BaseValueSegment(data_type_from_type<T>()), _values(pmr_vector<T>(0,&NVMMemoryResource::get())) {
...
ValueSegment<T>::ValueSegment(pmr_vector<T>&& values)
    : BaseValueSegment(data_type_from_type<T>()), _values(std::move(values), &NVMMemoryResource::get()) {}

template <typename T>
ValueSegment<T>::ValueSegment(pmr_vector<T>&& values, pmr_vector<bool>&& null_values)
    : BaseValueSegment(data_type_from_type<T>()), _values(std::move(values), &NVMMemoryResource::get()), _null_values(std::move(null_values)) {
