...
template <typename T>
struct Partition {
  pmr_vector<PartitionedElement<T>> elements{PolymorphicAllocator<void>{&NVMMemoryResource::get()}};
  ...
  // due to padding.
  pmr_vector<bool> null_values{PolymorphicAllocator<void>{&NVMMemoryResource::get()}};
};
...
class PosHashTable {
 ...
 public:
  explicit PosHashTable(const JoinHashBuildMode mode, const size_t max_size)
      : _hash_table(&NVMMemoryResource::get()), _pos_lists(max_size + 1, &NVMMemoryResource::get()), _mode(mode) {
