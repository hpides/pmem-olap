...
#ifdef __APPLE__
  Hyrise::get().plugin_manager.load_plugin("./lib/libhyriseTieringPlugin.dylib");
#elif __linux__
  //Hyrise::get().plugin_manager.load_plugin("./lib/libhyriseTieringPlugin.so");
#endif
...
void BenchmarkRunner::_create_report(std::ostream& stream) const {
  ...
  nlohmann::json report{{"context", _context},
                        {"benchmark_start",_benchmark_start.time_since_epoch().count()},
