 ...
 * FROM part[TENANT], supplier[TENANT], partsupp[TENANT], nation[TENANT], region[TENANT]
 * WHERE
 ...
 *        FROM partsupp[TENANT], supplier[TENANT], nation[TENANT], region[TENANT]
 *        WHERE
 *            p_partkey = ps_partkey
 *            AND s_suppkey = ps_suppkey
 *            AND s_nationkey = n_nationkey
 ...
 * LIMIT 100;
 *
 * The limit is not part[TENANT] of the printed query but hidden in the specification text.
 */
const char* const tpch_query_2 =
    R"(SELECT s_acctbal, s_name, n_name, p_partkey, p_mfgr, s_address, s_phone, s_comment
       ...
       ps_supplycost = (SELECT min(ps_supplycost) FROM partsupp[TENANT], supplier[TENANT], nation[TENANT], region[TENANT]
 ...
 * FROM customer[TENANT], orders[TENANT], lineitem[TENANT]
...
const char* const tpch_query_4 =
    R"(SELECT o_orderpriority, count(*) as order_count FROM orders[TENANT] WHERE o_orderdate >= ? AND
    o_orderdate < ? AND exists (
    SELECT * FROM lineitem[TENANT] WHERE l_orderkey = o_orderkey AND l_commitdate < l_receiptdate)
...
const char* const tpch_query_5 =
    R"(SELECT n_name, SUM(l_extendedprice * (1 - l_discount)) as revenue
      FROM customer[TENANT], orders[TENANT], lineitem[TENANT], supplier[TENANT], nation[TENANT], region[TENANT]
 ...
 *   FROM supplier[TENANT], lineitem[TENANT], orders[TENANT], customer[TENANT], nation[TENANT] n1, nation[TENANT] n2
 *   WHERE
 *       s_suppkey = l_suppkey
...
const char* const tpch_query_8 =
    R"(SELECT o_year, SUM(case when nation[TENANT] = ? then volume else 0 end) / SUM(volume) as mkt_share
     FROM (SELECT SUBSTR(o_orderdate, 1, 4) as o_year, l_extendedprice * (1-l_discount) as volume,
     n2.n_name as nation[TENANT] FROM part[TENANT], supplier[TENANT], lineitem[TENANT], orders[TENANT], customer[TENANT], nation[TENANT] n1, nation[TENANT] n2, region[TENANT]
 ...
 * FROM (
 *    SELECT
 *        n_name as nation[TENANT],
 *        extract(year FROM o_orderdate) as o_year,
 *        l_extendedprice * (1 - l_discount) - ps_supplycost * l_quantity as amount
 *    FROM part[TENANT], supplier[TENANT], lineitem[TENANT], partsupp[TENANT], orders[TENANT], nation[TENANT]
 *    WHERE
 *        s_suppkey = l_suppkey
 ...
 *    ) as profit
 * GROUP BY nation[TENANT], o_year
 * ORDER BY nation[TENANT], o_year DESC;
 *
 * Changes:
...
const char* const tpch_query_9 =
    R"(SELECT nation[TENANT], o_year, SUM(amount) as sum_profit FROM (SELECT n_name as nation[TENANT], SUBSTR(o_orderdate, 1, 4) as o_year,
      l_extendedprice * (1 - l_discount) - ps_supplycost * l_quantity as amount
      FROM part[TENANT], supplier[TENANT], lineitem[TENANT], partsupp[TENANT], orders[TENANT], nation[TENANT] WHERE s_suppkey = l_suppkey
      ...
      GROUP BY nation[TENANT], o_year ORDER BY nation[TENANT], o_year DESC;)";
...
const char* const tpch_query_10 =
    R"(SELECT c_custkey, c_name, SUM(l_extendedprice * (1 - l_discount)) as revenue, c_acctbal, n_name, c_address,
      c_phone, c_comment
      FROM customer[TENANT], orders[TENANT], lineitem[TENANT], nation[TENANT]
 ...
 * FROM partsupp[TENANT], supplier[TENANT], nation[TENANT]
 * WHERE
...
const char* const tpch_query_11 =
    R"(SELECT ps_partkey, SUM(ps_supplycost * ps_availqty) as value FROM partsupp[TENANT], supplier[TENANT], nation[TENANT]
      ...
      SELECT SUM(ps_supplycost * ps_availqty) * ? FROM partsupp[TENANT], supplier[TENANT], nation[TENANT]
 ...
 * FROM orders[TENANT], lineitem[TENANT]
 * WHERE
...
const char* const tpch_query_12 =
    R"(SELECT l_shipmode, SUM(case when o_orderpriority ='1-URGENT' or o_orderpriority ='2-HIGH' then 1 else 0 end)
      ...
      o_orderpriority <> '2-HIGH' then 1 else 0 end) as low_line_count FROM orders[TENANT], lineitem[TENANT]
 ...
 *    FROM customer[TENANT] left outer join orders[TENANT]
 *    on
 *        c_custkey = o_custkey
...
const char* const tpch_query_13 =
    R"(SELECT c_count, count(*) as custdist FROM (SELECT c_custkey, count(o_orderkey) AS c_count
      FROM customer[TENANT] left outer join orders[TENANT] on c_custkey = o_custkey AND o_comment not like ?
 ...
 * FROM lineitem[TENANT], part[TENANT]
 * WHERE
 *    l_partkey = p_partkey
...
const char* const tpch_query_14 =
    R"(SELECT 100.00 * SUM(case when p_type like 'PROMO%' then l_extendedprice*(1-l_discount) else 0 end)
      / SUM(l_extendedprice * (1 - l_discount)) as promo_revenue FROM lineitem[TENANT], part[TENANT] WHERE l_partkey = p_partkey
 ...
 * FROM supplier[TENANT], revenue[STREAM_ID]
 * WHERE
...
const char* const tpch_query_15 =
    R"(create view revenue_view (supplier_no, total_revenue) as SELECT l_suppkey,
      SUM(l_extendedprice * (1 - l_discount)) FROM lineitem[TENANT] WHERE l_shipdate >= '1996-01-01'
      AND l_shipdate < '1996-04-01' GROUP BY l_suppkey;

      SELECT s_suppkey, s_name, s_address, s_phone, total_revenue FROM supplier[TENANT], revenue_view
 ...
 * FROM partsupp[TENANT], part[TENANT]
 * WHERE
 *    p_partkey = ps_partkey
 *    AND p_brand <> '[BRAND]'
...
const char* const tpch_query_16 =
    R"(SELECT p_brand, p_type, p_size, count(distinct ps_suppkey) as supplier_cnt
      FROM partsupp[TENANT], part[TENANT] WHERE p_partkey = ps_partkey AND p_brand <> ?
      AND p_type not like ? AND p_size in (?, ?, ?, ?, ?, ?, ?, ?)
      AND ps_suppkey not in (SELECT s_suppkey FROM supplier[TENANT] WHERE s_comment like '%Customer%Complaints%')
...
const char* const tpch_query_17 =
    R"(SELECT SUM(l_extendedprice) / 7.0 as avg_yearly FROM lineitem[TENANT], part[TENANT] WHERE p_partkey = l_partkey
      AND p_brand = ? AND p_container = ? AND l_quantity < (SELECT 0.2 * avg(l_quantity)
      FROM lineitem[TENANT] WHERE l_partkey = p_partkey);)";
...
const char* const tpch_query_18 =
    R"(SELECT c_name, c_custkey, o_orderkey, o_orderdate, o_totalprice, SUM(l_quantity)
      FROM customer[TENANT], orders[TENANT], lineitem[TENANT] WHERE o_orderkey in (SELECT l_orderkey FROM lineitem[TENANT]
...
const char* const tpch_query_19 =
    ...
    R"(SELECT SUM(l_extendedprice * (1 - l_discount) ) as revenue FROM lineitem[TENANT], part[TENANT] WHERE (( p_partkey = l_partkey AND
 ...
 * FROM supplier[TENANT], nation[TENANT]
 * WHERE
 *    s_suppkey in (
 *        SELECT ps_suppkey
 *        FROM partsupp[TENANT]
 *        WHERE ps_partkey in (
 *          SELECT p_partkey
 *          FROM part[TENANT]
 *          WHERE
...
const char* const tpch_query_20 =
    R"(SELECT s_name, s_address FROM supplier[TENANT], nation[TENANT] WHERE s_suppkey in (SELECT ps_suppkey FROM partsupp[TENANT]
      WHERE ps_partkey in (SELECT p_partkey FROM part[TENANT] WHERE p_name like ?) AND ps_availqty >
      (SELECT 0.5 * SUM(l_quantity) FROM lineitem[TENANT] WHERE l_partkey = ps_partkey AND l_suppkey = ps_suppkey AND
 ...
 * FROM supplier[TENANT], lineitem[TENANT] l1, orders[TENANT], nation[TENANT]
 * WHERE
 ...
 *    AND exists (
 *        SELECT *
 *        FROM lineitem[TENANT] l2
 *        WHERE
 *            l2.l_orderkey = l1.l_orderkey
 *            AND l2.l_suppkey <> l1.l_suppkey
 *    ) AND not exists (
 *        SELECT *
 *        FROM lineitem[TENANT] l3
 *        WHERE
...
const char* const tpch_query_21 =
    R"(SELECT s_name, count(*) as numwait FROM supplier[TENANT], lineitem[TENANT] l1, orders[TENANT], nation[TENANT] WHERE s_suppkey = l1.l_suppkey
      AND o_orderkey = l1.l_orderkey AND o_orderstatus = 'F' AND l1.l_receiptdate > l1.l_commitdate AND exists
      (SELECT * FROM lineitem[TENANT] l2 WHERE l2.l_orderkey = l1.l_orderkey AND l2.l_suppkey <> l1.l_suppkey) AND not exists
      (SELECT * FROM lineitem[TENANT] l3 WHERE l3.l_orderkey = l1.l_orderkey AND l3.l_suppkey <> l1.l_suppkey AND
          ...
          WHERE
            ...
            c_acctbal > (SELECT
                         ...
                         WHERE
                          c_acctbal > 0.00 AND
                          SUBSTR(c_phone,1,2) IN (?, ?, ?, ?, ?, ?, ?)) AND
            NOT EXISTS ( SELECT * FROM orders[TENANT] WHERE o_custkey = c_custkey)
        ) AS custsale
       GROUP BY cntrycode
       ORDER BY cntrycode;)";
