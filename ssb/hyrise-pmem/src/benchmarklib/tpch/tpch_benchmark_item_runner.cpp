...
TPCHBenchmarkItemRunner::TPCHBenchmarkItemRunner(const std::shared_ptr<BenchmarkConfig>& config,
                                                 bool use_prepared_statements, float scale_factor,
                                                 const std::vector<BenchmarkItemID>& items, const int tenants)
    : AbstractBenchmarkItemRunner(config),
      _use_prepared_statements(use_prepared_statements),
      _scale_factor(scale_factor),
      _items(items),
...
std::string TPCHBenchmarkItemRunner::_build_query(const BenchmarkItemID item_id) {
  ...
  switch (item_id) {
    ...
    case 15 - 1: {
      ...
      static constexpr auto BEGIN_DATE_OFFSET = 164;
      static constexpr auto END_DATE_OFFSET = 200;
      ...
      static thread_local std::minstd_rand random_engine{_random_seed++};
      std::uniform_int_distribution<> tenant_dist{0, _tenants - 1};
      const auto tenant = tenant_dist(random_engine);
      boost::replace_all(query_15, std::string("[TENANT]"), std::to_string(tenant));
...
std::string TPCHBenchmarkItemRunner::_substitute_placeholders(const BenchmarkItemID item_id,
  ...
  if (_use_prepared_statements) {
    Fail("Prepared statements are not supported right now as the tenant substition is not implemented yet");
    ...
    std::uniform_int_distribution<> tenant_dist{0, _tenants - 1};
    const auto tenant = tenant_dist(random_engine);
    boost::replace_all(query_template, std::string("[TENANT]"), std::to_string(tenant));
